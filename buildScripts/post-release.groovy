            ...
            steps {
                dir("${params.module}") {
                    ...
                    sshagent(['github-bot-ssh']) {
                        ...
                        '''
                        sh "mvn --batch-mode -s /home/jenkins/.m2/settings.xml -Ppromote-stage -Drelease -Dnexus.staging.repository=${params.module}-maven2-staging -DskipTests -Dmaven.deploy.skip=false deploy"
                    }
                }
            }
        }
        stage("Move Specs From Staging") {
            ...
            steps {
                sshagent ( ['projects-storage.eclipse.org-bot-ssh']) {
                    sh "ssh genie.microprofile@projects-storage.eclipse.org [ -e /home/data/httpd/download.eclipse.org/microprofile/staging/${params.module}-${params.releaseVersion} ] && ssh genie.microprofile@projects-storage.eclipse.org mv /home/data/httpd/download.eclipse.org/microprofile/staging/${params.module}-${params.releaseVersion} /home/data/httpd/download.eclipse.org/microprofile/ || echo 'Staging directory ${params.module}-${params.releaseVersion} not found, skipping move (may have already been moved)'"
                }
            }
        }
    }
